...
@section('body')
    <div class="col-md-10 offset-md-1" style="border-bottom: 1px solid #c8c8c8;border-right:1px solid #c8c8c8 ;box-shadow:  5px 10px 18px #888888;padding-top: 1%; ">
        {{Form::open(['route'=>'defendeur.create_defendeur'])}}
        ...
        <div class="form-group col-md-6">
            <dl class="row">
                <dt class="col-sm-8">{{Form::text('nom_defendeur',null,['class'=>'form-control'])}}</dt>
                <dd class="col-sm-4">{{Form::label('nom_defendeur',':الاسم الكامل')}}</dd>
            </dl>
        </div><div class="form-group col-md-6">
            <dl class="row">
                <dt class="col-sm-8">{{Form::text('adresse_defendeur',null,['class'=>'form-control'])}}</dt>
        ...
        $(function () {
            $('#yes').on('click',function () {
                ...
                $('#form_av').addClass('col-md-6').append('<div class="bubble"><a class="close_span" style="margin-left:110%;cursor:pointer;" ><i class="fa fa-times fa-lg" ></i></a><div class="form-group offset-md-1">' +
                    '<dl class="row">' +
                    '{{Form::select('genre_d[]',['M'=>'رجل','F'=>'امرأة'],null,['class'=>'form-control col-md-6'])}}' +
                    '&nbsp;&nbsp;&nbsp;&nbsp;{{Form::label('genre_d',': رجل/امرأة  ')}}' +
                    '</dl>' +
                    ' </div>' +
...
//                $('#av_fild').fadeOut()
            ...
            $('#add_av').on('click',function () {
                ...
                $('#form_av').addClass('col-md-6').append('<div class="bubble"><a class="close_span" style="margin-left:110%;cursor:pointer;" ><i class="fa fa-times fa-lg" ></i></a><div class="form-group offset-md-1">' +
                    ...
                    ' <div class="form-group">' +
                    '<dl class="row">' +
                    '<dt class="col-sm-8">{{Form::text('nom_avocat[]',null,['class'=>'form-control','id'=>'nom_avocat.1'])}}</dt>' +
                    '<dd class="col-sm-4">{{Form::label('nom_avocat',':الإسم الكامل ')}}</dd>'+
                    '</dl>'+
                    '</div>'+
                    '<div class="form-group">'+
                    ' <dl class="row">'+
                    ' <dt class="col-sm-8">{{Form::textarea('adresse_avocat[]',null,['class'=>'form-control','id'=>'adresse_avocat.1'])}}</dt>'+
                    '<dd class="col-sm-4">{{Form::label('adresse_avocat',':العنوان ')}}</dd>'+
                    '</dl>'+
                    '</div>'+
                    ' <div class="form-group">'+
                    '<dl class="row">'+
                    '<dt class="col-sm-8">{{Form::text('ville_avocat[]',null,['class'=>'form-control','id'=>'ville_avocat.1'])}}</dt>'+
                    '<dd class="col-sm-4">{{Form::label('ville_avocat',':المدينة ')}}</dd>'+
                    '</dl>'+
                    '</div>'+
                    '</div>');
